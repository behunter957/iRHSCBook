...
@objc class RHSCCourtTime : NSObject {
    ...
    init(withJSONDictionary jsonDictionary:[String: Any], forUser userId:String, members:RHSCMemberList) {
        super.init()
        // Assign all properties with keyed values from the dictionary
        bookingId = nullToString(jsonDictionary["booking_id"] as? String)
        court = nullToString(jsonDictionary["court"] as? String)
        courtDateStr = nullToString(jsonDictionary["courtdate"] as? String)
        courtTimeStr = nullToString(jsonDictionary["courttime"] as? String)
        let dateFormat = DateFormatter()
        dateFormat.dateFormat = "yyyy-MM-dd HH:mm:ss"
        courtTime = dateFormat.date(from: String.init(format: "%@ %@", arguments:
            [ courtDateStr!, courtTimeStr! ]))!
        status = nullToString(jsonDictionary["courtStatus"] as? String)
        event = nullToString(jsonDictionary["courtEvent"] as? String)
        eventDesc = nullToString(jsonDictionary["eventDesc"] as? String)
        isNoShow = (nullToString(jsonDictionary["noshow"] as? String) == "true")
        let ml = members.memberDict
        bookedForUser = false;
        let player1_id = nullToString(jsonDictionary["player1_id"] as? String)
        let player2_id = nullToString(jsonDictionary["player2_id"] as? String)
        let player3_id = nullToString(jsonDictionary["player3_id"] as? String)
        let player4_id = nullToString(jsonDictionary["player4_id"] as? String)
        let g2name = nullToString(jsonDictionary["g2_name"] as? String)
        let g3name = nullToString(jsonDictionary["g3_name"] as? String)
        let g4name = nullToString(jsonDictionary["g4_name"] as? String)
    ...
    func book(fromView view: UIViewController) {
        ...
        let tbc = view.tabBarController as! RHSCTabBarController
        let g2name = players[2] is RHSCGuest ? (players[2] as! RHSCGuest).guestName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) : ""
        let g3name = players[3] is RHSCGuest ? (players[3] as! RHSCGuest).guestName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) : ""
        let g4name = players[4] is RHSCGuest ? (players[4] as! RHSCGuest).guestName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) : ""
        ...
        let urlstr = String.init(format: "Reserve20/IOSBookCourtJSON.php?booking_id=%@&player1_id=%@&player2_id=%@&player3_id=%@&player4_id=%@&uid=%@&channel=%@&guest2=%@&guest3=%@&guest4=%@&channel=%@&court=%@&courtEvent=%@&reserved=false",
            arguments: [bookingId!,
                ...
                "iPhone", court!.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!,
                event!])
        //        print(urlstr)
        let url = URL(string: urlstr, relativeTo: tbc.server?.absoluteURL )
